...
L’API CMU fournit une vue hiérarchique des cubes OLAP sous-jacents. Chaque ressource ([dimension](/help/concurrency-monitoring/cm-usage-reports.md#dimensions-2-filter-metrics) de la hiérarchie des dimensions, mappée en tant que segment de chemin d’URL) génère des rapports avec des [mesures](/help/concurrency-monitoring/cm-usage-reports.md#monitor-metrics) (agrégées) pour la sélection en cours. Chaque ressource pointe vers sa ressource parent (pour le cumul) et ses sous-ressources (pour l’exploration vers le bas). Le découpage et la division sont réalisés au moyen de paramètres de chaîne de requête qui épinglent des dimensions à des valeurs ou des plages spécifiques.
