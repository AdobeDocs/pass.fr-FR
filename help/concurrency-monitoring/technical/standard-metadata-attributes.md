...
L’API de surveillance de concurrence force les clients à envoyer les valeurs suivantes dans le cadre de tout appel d’initialisation valide : [appels d’initiation de session](/help/concurrency-monitoring/technical/restrict-concurr-usage-mult-apps.md#api-calls-descr).
...
Avec [API v2.0](https://streams-stage.adobeprimetime.com/swagger-ui/index.html), si l’un de ces attributs est requis par les politiques définies, une tentative d’initialisation de session sans cet attribut entraînera une requête incorrecte de 400.
