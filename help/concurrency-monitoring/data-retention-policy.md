...
*Données Clickstream* représente les données partagées par les clients sur les pulsations de la session (par exemple, subjectID, mvpdName et metadata). Tous les champs de métadonnées personnalisés sont référencés dans la [Attributs de métadonnées standard](/help/concurrency-monitoring/standard-metadata-attributes.md).
