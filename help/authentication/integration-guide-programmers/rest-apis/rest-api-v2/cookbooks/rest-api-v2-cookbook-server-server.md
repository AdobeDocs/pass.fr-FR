...
Pour obtenir le jeton d’accès, le service de programmation peut agir au nom d’une application de diffusion en continu et doit suivre les étapes décrites dans la documentation [Enregistrement dynamique du client](../../rest-api-dcr/apis/dynamic-client-registration-apis-retrieve-access-token.md).
...
Avec un profil d’authentification valide pour un utilisateur, le service de programmation a la possibilité de vérifier l’accès aux vidéos disponibles et de transmettre la liste à l’application de diffusion en continu à afficher.
