...
Pour utiliser l’API REST d’authentification Adobe Pass, les périphériques doivent satisfaire ou dépasser les exigences techniques minimales répertoriées dans la section API REST de la variable [Document sur la plateforme d’authentification Adobe Pass/les exigences des appareils/outils](#general_clientless_reqs).
